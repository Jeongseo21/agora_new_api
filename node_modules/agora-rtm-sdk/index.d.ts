...
interface AttributesMap {
  ...
  /** @zh-cn
  * 属性名和属性值，以键值对形式表示。单个属性名和属性值的总长度不得超过 8 KB。单个属性名长度不得超过 32 字节。 
  */
  /**
   * Attribute name and attribute value in the form of a key value pair. The total length of a single attribute name and attribute value must not exceed 8 KB. The length of a single attribute name must not exceed 32 bytes.
   */
  [key: string]: string;
}
/** @zh-cn
...
declare namespace RtmEvents {
  ...
  export interface RtmChannelEvents {
     ...
     * <p><b>Note</b></p>
     * 
     * <li> 频道成员人数 &le; 512 时，最高触发频率为每秒 1 次。</li>
     * <li> 频道成员人数超过 512 时，最高触发频率为每 3 秒 1 次。</li>
     ...
     * <p><b>Note</b></p>
     * 
     * <li> When the number of channel members &le; 512, the SDK returns this callback when the number changes and at a MAXIMUM speed of once per second. </li>
     * <li> When the number of channel members exceeds 512, the SDK returns this callback when the number changes and at a MAXIMUM speed of once every three seconds. </li>
 ...
 *
 * 可在初始化时通过 {@link createInstance} 的第 2 个参数或实例上的 {@link setParameters} 方法进行设置。
 */
/**
 * Interface holding the configurable parameters of an `RtmClient` instance.
 *
 * You can pass it as the second argument when calling the {@link createInstance} method, or use it when calling the {@link setParameters} method.
 */
...
declare class RtmClient extends EventEmitter<RtmEvents.RtmClientEvents> {
  /** @zh-cn
   * 用户登录 Agora RTM 系统。
   * @note Agora 不推荐在同时进行 RTM 和 RTC 系统的登录操作。在弱网环境下，同时进行 RTM 和 RTC 系统的登录操作可能会大大降低登录成功率。
   ...
   *
   * @note Agora does not recommend logging in to the Agora RTM system and the Agora RTC system concurrently. Under poor network conditions, concurrent login operations to the Agora RTM system and the Agora RTC system are much more likely to fail.
   ...
   * <p>发送消息（包括点对点消息和频道消息）的频率上限为 60 次每秒。</p>
   * @example
   * ```TypeScript
   ...
   * <p>You can send messages, including peer-to-peer and channel messages at a maximum frequency of 60 calls per second.</p>
   * @example
   * ```TypeScript
   * client.sendMessageToPeer(
   ...
   * <p><b>Note</b></p>
   * <ul>
   * <li>You do not have to join the specified channel to clear its attributes.</li>
   * <li>For {@link RtmClient.setChannelAttributes}, {@link addOrUpdateChannelAttributes}, {@link deleteChannelAttributesByKeys}, and {@link clearChannelAttributes} taken together: the call frequency limit is 10 queries every five seconds.</li>
   * </ul>
   ...
   * <li> For {@link setLocalUserAttributes}, {@link addOrUpdateChannelAttributes}, {@link deleteChannelAttributesByKeys}, and {@link clearChannelAttributes} taken together: the call frequency limit is 10 queries every five seconds.</li>
   * </ul>
   * @param channelId The channel ID of the specified channel.
...
declare namespace AgoraRTM {
   ...
   * @param params {@link RtmClient} 对象的配置参数。
   ...
   * @param params The configurable parameters of an {@link RtmClient} instance.
